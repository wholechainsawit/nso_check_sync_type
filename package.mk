...
PACKAGE_MK_VERSION = 1.0.0-147-g18240f3
...
# --- Features -----------------------------------------------------------------
JAVA_ENABLED := $(if $(wildcard $(JAVA_DIR)/build.xml),true,)
...
$(LOAD_DIR)/%.fxs: $(YANG_DIR)/%.yang | $(LOAD_DIR) $(PACKAGE_MK_TMP_LOCAL)/deps/yang/yang
	$(VERBOSE) $(NCSC) --use-description $(YANG_PATHS:%=--yangpath %) -c $< -o $@
	$(VERBOSE) $(PYANG) -f depend $(YANG_PATHS:%=-p %) --depend-include-path --depend-target $@ --ignore-errors -o $(PACKAGE_MK_TMP_LOCAL)/deps/yang/yang/$(rel)$(basename $(notdir $<)).yang.dep $<

$(LOAD_DIR)/%.fxs: $(NCSC_OUT_DIR)/modules/fxs/%.fxs | $(LOAD_DIR) $(PACKAGE_MK_TMP_LOCAL)/deps/yang/ned
	$(VERBOSE) cp $< $@
	$(VERBOSE) $(PYANG) -f depend $(YANG_PATHS:%=-p %) --depend-include-path --depend-target $@ --ignore-errors -o $(PACKAGE_MK_TMP_LOCAL)/deps/yang/ned/$(rel)$(basename $(notdir $@)).yang.dep $(YANG_NED_DIR)/$(basename $(notdir $@)).yang
...
RELEASE_PATHS  += CHANGES.txt erlang-lib/*/ebin/ load-dir/ package-meta-data.xml private-jar/ shared-jar/ python/ templates/ webui/ src/yang src/ncsc-out
...
# build-meta-data.xml is created at later point in time, hence may not be part of RELEASE_PATHSSS when resolved
	$(VERBOSE) $(TAR) -C $(PACKAGE_DIR_ABS)/.. -czf $@ \
		...
		$(if $(shell ls $(PACKAGE_DIR_ABS)/build-meta-data.xml 2> /dev/null),$(addprefix $(notdir $(PACKAGE_DIR_ABS))/,build-meta-data.xml),)
...
ifeq ($(SKIP_DEPS),)
bundle-release-deps: $(build_deps_dirs) clean-deps-log build-meta-data | $(PACKAGE_MK_TMP)
